...
export class GameBoardPage implements OnInit {
  ...
  // Show the graveyard modal
  showGraveyard() {
    this.graveyardModalVisible = true;
  }

  // Close the graveyard modal
  closeGraveyard() {
    this.graveyardModalVisible = false;
  }
  ...
  if (isPlayer) {
    ...
    // Add the discarded card to the player's graveyard
  ...
  if (this.lifePoints <= 0) {
    ...
    of(null).pipe(delay(2000)).subscribe(() => {
      this.router.navigate(['/game'], {
        state: { duelResult: 'Win' } // Pass the result back to GamePage
      });
    });
  }
}
  ...
  if (!this.playerTurn) {
    ...
    if (this.opponentHand.length > 0 && this.opponentMonsterCards.length < 5) {
      ...
      // Remove card from opponent's hand and update the array reference
