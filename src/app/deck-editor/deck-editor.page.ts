...
interface Card {
  ...
  name: string;
  imageUrl: string;
  atk?: number; // Optional attack property
  def?: number; // Optional defense property
  type?: string; // Optional type property
  attribute?: string; // Optional attribute property
  level?: number; // Optional level property
  race?: string; // Optional race property
  archetype?: string; // Optional archetype property
  desc?: string; // Optional description property
  card_images?: { image_url: string }[]; // Optional card images property
  card_prices?: any[]; // Optional card prices property
  card_sets?: any[]; // Optional card sets property
}
  ...
  async loadDecks(): Promise<void> {
    try {
      ...
      this.decks = (decks ?? []).map((deck) => ({
        ...deck,
        cards: deck.cards.map((card: any) => this.normalizeCard(card)), // Normalize cards
      }));
      console.log('Decks loaded and normalized:', this.decks);
    } catch (error) {
  ...
  this.http.get<any>(apiUrl).subscribe({
    next: (response) => {
      ...
      if (response && response.data) {
        this.allCards = response.data.map((card: any) => this.normalizeCard(card));
      } else {
        this.allCards = [];
...
private normalizeCard(card: any): Card {
  return {
    ...
    card_images: card.card_images || [{ image_url: 'https://via.placeholder.com/150' }],
  ...
  addToDeck(card: Card): void {
    ...
    if (cardCount < 3) {
      const normalizedCard = this.normalizeCard(card); // Normalize the card before adding
      this.selectedDeck.cards.push(normalizedCard);
      this.saveSelectedDeck(); // Save the updated selected deck to localStorage
      this.saveDecks(); // Save all decks to localStorage
      this.deckService.updateDeck(this.selectedDeck, normalizedCard); // Update Firestore
      console.log(`Card "${normalizedCard.name}" added to deck "${this.selectedDeck.name}".`);
    } else {
